...
export default function DevChatMobileDev() {
  ...
  // Detect keyboard visibility and adjust padding
  useEffect(() => {
    const handleResize = () => {
      if (typeof window !== 'undefined' && window.visualViewport) {
        ...
        // Keyboard is visible if viewport shrinks significantly (>100px threshold)
        const keyboardVisible = (windowHeight - viewportHeight) > 100
        setIsKeyboardVisible(keyboardVisible)

        // Scroll to bottom when keyboard appears
        if (messagesContainerRef.current && keyboardVisible) {
