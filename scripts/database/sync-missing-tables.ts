#!/usr/bin/env tsx
/**
 * Sync specific missing tables from production to staging
 *
 * This script specifically syncs:
...
async function syncTable(tableName: string, primaryKey: string = 'id'): Promise<number> {
  console.log(`\n📋 Syncing ${tableName}...`);
  ...
  let totalCopied = 0;
  const batchSize = 500; // Smaller batch for large tables
  let offset = 0;
  let hasMore = true;
...
async function main() {
  console.log('');
  console.log('🔧 Syncing Missing Tables from Production → Staging');
  console.log('==================================================');
  ...
  // 1. Sync code_embeddings (no foreign keys, safe to sync)
  const embeddings = await syncTable('code_embeddings', 'id');
  totalRows += embeddings;

  // 2. Sync muva_content (no foreign keys, safe to sync)
  const content = await syncTable('muva_content', 'id');
  totalRows += content;

  // 3. Sync guest_reservations (has foreign keys, might fail partially)
  console.log('\n⚠️ Note: guest_reservations has foreign keys - some rows may fail');
  const reservations = await syncTable('guest_reservations', 'id');
  totalRows += reservations;
  ...
  const tables = ['code_embeddings', 'muva_content', 'guest_reservations'];
