...
**Proyecto:** Migración MUVA Chat.io → MUVA.chat
**Fecha:** 2025-10-10
...
- [x] Listar certificados SSL activos en VPS (estimate: 10min)
  ...
  - Verificar wildcard cert para `*.muva.chat` existe
...
- [x] Crear backup de configs antes de modificar (estimate: 10min)
  - Copiar `/etc/nginx/sites-available/muva.chat` → `backups/nginx-subdomain.conf.backup`
...
- [x] Encontrar todas las referencias a `muva.chat` en codebase (estimate: 15min)
...
- [x] Capturar logs actuales como baseline (estimate: 5min)
  - PM2 logs: `pm2 logs muva-chat --lines 100 > fase-0/pm2-baseline.log`
  - Nginx access: `sudo tail -n 200 /var/log/nginx/muva-subdomain-access.log > fase-0/nginx-access-baseline.log`
  - Nginx error: `sudo tail -n 100 /var/log/nginx/muva-subdomain-error.log > fase-0/nginx-error-baseline.log`
...
- [x] Agregar detección de subdomain para .muva.chat (estimate: 20min)
  ...
  - Mantener estructura idéntica a lógica `.muva.chat`
...
- [ ] Validar getSubdomain() detecta ambos dominios (estimate: 10min)
  ...
  - Test 1: `getSubdomain('simmerdown.muva.chat')` → 'simmerdown'
  - Test 2: `getSubdomain('simmerdown.muva.chat')` → 'simmerdown'
  - Test 3: `getSubdomain('muva.chat')` → null
  - Test 4: `getSubdomain('muva.chat')` → null
...
- [ ] Actualizar server_name y regex en Nginx config (estimate: 15min)
  ...
  - Modificar línea 9: `server_name *.muva.chat muva.chat *.muva.chat muva.chat;`
...
- [ ] Copiar nginx config y reload service (estimate: 10min)
  - SSH a VPS
  - `sudo cp /var/www/muva-chat/docs/deployment/nginx-subdomain.conf /etc/nginx/sites-available/muva.chat`
  - `sudo nginx -t` (test config syntax)
  - `sudo systemctl reload nginx` (graceful reload)
  - Verificar no errores en syslog
  - Files: `/etc/nginx/sites-available/muva.chat` (VPS)
  - Agent: **@agent-deploy-agent**
...
- [ ] Verificar SSL y routing funcionan (estimate: 15min)
  ...
  - Test 2: `curl -I https://simmerdown.muva.chat`
...
- [ ] Revisar logs por 1 hora para detectar errores (estimate: 60min)
  - Monitor PM2: `pm2 logs muva-chat --lines 100`
  - Monitor Nginx access: `sudo tail -f /var/log/nginx/muva-subdomain-access.log`
  - Monitor Nginx error: `sudo tail -f /var/log/nginx/muva-subdomain-error.log`
...
- [ ] Agregar permanent redirect muva.chat → muva.chat (estimate: 20min)
  ...
  - Agent: **@agent-deploy-agent**
  - Test: `curl -I https://simmerdown.muva.chat` debe retornar 301

### 4.2 Test Redirects
- [ ] Validar todos los redirects funcionan (estimate: 10min)
  - Test: `curl -I https://muva.chat` → 301 to muva.chat
  - Test: `curl -I https://simmerdown.muva.chat` → 301 to simmerdown.muva.chat
  - Test: `curl -I https://free-hotel-test.muva.chat` → 301
  - Test: `curl -I https://hotel-boutique.muva.chat` → 301
...
- [x] Update all docs references muva.chat → muva.chat (estimate: 15min) ✅
