-- Migration: Fix unit_type mapping in sync function
-- Date: 2025-10-23
-- Purpose: Map 'accommodation' to proper unit_type values (room/apartment/suite)
...
BEGIN
  ...
  LOOP
    BEGIN
      -- Map unit_type: Check capacity to determine room vs apartment
      -- If capacity > 4 → apartment, otherwise → room
      v_unit_type := CASE
        WHEN (v_unit.metadata->'capacity'->>'total')::INTEGER > 4 THEN 'apartment'
        WHEN v_unit.metadata->>'accommodation_mphb_type' LIKE '%10 personas%' THEN 'apartment'
        WHEN v_unit.metadata->>'accommodation_mphb_type' LIKE '%apartamento%' THEN 'apartment'
        ELSE 'room'
      END;
...
'Syncs consolidated accommodation units from accommodation_units_public to hotels.accommodation_units with proper unit_type mapping (room/apartment/suite based on capacity).';
