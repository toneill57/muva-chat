-- Migration: Add RLS policies for SIRE data access
-- Date: 2025-10-09
...
CREATE POLICY "Tenant users can insert their own reservations"
  ...
  WITH CHECK (
    tenant_id IN (
      ...
      WHERE utp.user_id = auth.uid()
        AND utp.is_active = true
        AND utp.permission_level IN ('owner', 'admin', 'staff')
    )
  );
...
CREATE POLICY "Tenant owners can delete their own reservations"
  ...
  USING (
    tenant_id IN (
      ...
      WHERE utp.user_id = auth.uid()
        AND utp.is_active = true
        AND utp.permission_level IN ('owner', 'admin')
    )
  );
...
BEGIN
  ...
  -- Check if user has active permission for the tenant
  RETURN EXISTS (
    SELECT 1
    FROM user_tenant_permissions utp
