/**
 * Remove is_active filter from match_accommodations_hybrid
 *
 * REASON: Allow inactive accommodations to show in search results.
 * The UI will display status badge (active/inactive) so users can see the difference.
 *
 * CHANGE: Remove lines:
 *   AND aup.is_active = true
 *   AND aup.is_bookable = true
 *
 * This allows ALL tenant accommodations to appear in search, regardless of status.
 */
...
'Hybrid multi-tier vector search for PUBLIC chat (INCLUDES INACTIVE accommodations).
